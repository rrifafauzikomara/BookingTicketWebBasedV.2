        ...
        <div id="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                <!-- /.row -->
                    ...
                    $no_pesanm = @$_GET['id'];
                    $aksi = @$_GET['aksi'];
                    if(($aksi<>"") and ($no_pesanm<>"")){
                    $sql_delete = "delete from tb_member where no_pesanm='$no_pesanm'" or die (mysqli_error());
                    $delete = mysqli_query($con, $sql_delete);
                    echo "<script type=text/javascript>window.location.href='http://localhost/primajasa/admin/databookingmember.php';</script>" ;
                                    }
                                    ?>
                        ...
                        <table id="dataTable" class="table table-bordered table-condensed table-hover table-striped">
                            ...
                            <?php
                            $sql = "select * from tb_member" or die (mysql_error());
                            $tampil = mysqli_query($con, $sql);
                            while($data = mysqli_fetch_array($tampil)) {        
                            ?>

                            <tr>
                                <td><?php echo $data['no_pesanm']; ?></td>
                                <td><?php echo $data['namam']; ?></td>
                                <td><?php echo $data['nama2ndm']; ?></td>
                                <td><?php echo $data['alamatm']; ?></td>
                                <td><?php echo $data['tgl_kebm']; ?></td>
                                <td><?php echo $data['seatm']; ?></td>
                                <td><?php echo $data['statusm']; ?></td>
                                <td align="center">
                                <a  href="editbookingmember.php?id=<?php echo $data['no_pesanm']; ?>" class="btn btn-xs btn-default" title="Edit" ><i class="fa fa-edit fa-fw"></i></a>
                                <a onclick="return confirm('Anda yakin akan menghapus');" href="databookingmember.php?id=<?php echo $data['no_pesanm']; ?>&aksi=hapus" class="btn btn-xs btn-default" title="Hapus"><i class="fa fa-times fa-fw"></i></a>
                                </td>
                            </tr>
